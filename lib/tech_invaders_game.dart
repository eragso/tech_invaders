...
class TechInvadersGame extends BaseGame with Resizable, TapDetector {
  ...
  TechInvadersGame(Size size) {
    ...
    tileSize = (size.width -
            ((_invadersPerRow + edgeSpacingPerRow - 1) * _defaultSpacing)) /
        (_invadersPerRow + edgeSpacingPerRow);
    _initOffsetX = (tileSize * (edgeSpacingPerRow / 2)) +
        (((edgeSpacingPerRow / 2) - 1) * _defaultSpacing);
  ...
  void startGame() async {
    ...
    Flame.audio.loopLongAudio("song.wav");
    gameInProgress = true;
    gameOver = false;
    shouldUpdate = true;
  ...
  void startNewLevel() {
    _initialize();
    base1 == null
        ? addLater(base1 = Base(tileSize * 2, 1, 1, screenSize))
        : updateBase(1, 1);
    base1Y = base1.y;
    base2 == null
        ? addLater(base2 = Base(tileSize * 2, 1, 2, screenSize))
        : updateBase(2, 1);
    base3 == null
        ? addLater(base3 = Base(tileSize * 2, 1, 3, screenSize))
        : updateBase(3, 1);
    base4 == null
        ? addLater(base4 = Base(tileSize * 2, 1, 4, screenSize))
        : updateBase(4, 1);
    invadersKilled = 0;
  ...
  void _initialize() async {
    ...
    for (int r = 0; r < 5; r++) {
      for (int c = 0; c < 11; c++) {
        ...
        double y = _initOffsetY + (r * tileSize) + (r * _defaultSpacing) - 90;
  ...
  void updateBase(int whichBase, int whatPhase) {
    switch (whichBase) {
      case (1):
        base1 = null;
        if (whatPhase < 20)
          addLater(base1 = Base(tileSize * 2, whatPhase, 1, screenSize));
        break;
      case (2):
        base2 = null;
        if (whatPhase < 20)
          addLater(base2 = Base(tileSize * 2, whatPhase, 2, screenSize));
        break;
      case (3):
        base3 = null;
        if (whatPhase < 20)
          addLater(base3 = Base(tileSize * 2, whatPhase, 3, screenSize));
        break;
      case (4):
        base4 = null;
        if (whatPhase < 20)
          addLater(base4 = Base(tileSize * 2, whatPhase, 4, screenSize));
        break;
      default:
        break;
    }
